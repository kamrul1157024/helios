...
func (h *Host) serveConn(conn net.Conn) {
	...
	if hello.Role != RoleInteractive && hello.Role != RoleObserver {
	...
	if !replayed {
		seq := h.ring.Seq()
		if err := WriteFrame(conn, FrameSnapshot,
			EncodeSnapshot(seq, []byte(h.screen.RenderSnapshot(SnapshotScrollbackLines)))); err != nil {
			return
		}
	}
