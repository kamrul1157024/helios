...
type sseNotification struct {
	...
	// Terminal is the session's live terminal handle, enriched by the daemon.
...
func handleNotificationEvent(bin, data string, settings map[string]string) {
	...
	notifLog.Printf("notification: id=%s type=%s terminal=%s", notif.ID, notif.Type, notif.Terminal)
	...
	notifLog.Printf("firing: os=%s bin=%s body=%q sound=%v", runtime.GOOS, bin, truncate(body, 80), sound)

	switch runtime.GOOS {
	case "darwin":
		sendDarwin(bin, notif.ID, notif.Type, body, sound)
	case "linux":
		sendLinux(bin, notif.Type, body)
	}
}

func sendDarwin(bin, id, notifType, body string, sound bool) {
	args := []string{
		"-title", "Helios",
